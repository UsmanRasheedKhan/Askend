...
const ProfileCompletionScreen = ({ navigation }) => {
    ...
    // Dropdown open/close state (Same as before)
    const [genderOpen, setGenderOpen] = useState(false);
    const [maritalOpen, setMaritalOpen] = useState(false);

    // Error states (Same as before)
    ...
    // ----------------------------------------------------
    // SAVE HANDLER (UPDATED FOR REST API)
    // ----------------------------------------------------
    const handleSave = async () => {
        ...
        try {
            // 1. Get current session/access token from Supabase client
            const {
                data: { session },
                error: sessionError,
            ...
            const formattedDobString = dob.toISOString().split("T")[0]; // Format as 'YYYY-MM-DD'

            const payload = {
                user_id: session.user.id, // User ID from the session is crucial for RLS
                full_name: fullName.trim(),
                gender: gender,
                date_of_birth: formattedDobString,
                marital_status: maritalStatus,
                // ⭐ CRITICAL FIX: The column 'profile_creation_step' was removed 
                // as it does not exist in the database table schema.
            };

            // 3. Perform the REST API INSERT (POST request)
            const response = await fetch(`${REST_API_URL}?select=id`, {
                method: "POST",
                // Ensure API_HEADERS includes 'Authorization': `Bearer ${session.access_token}` and 'Content-Type': 'application/json'
            ...
            if (response.ok) {
                ...
                if (data && data.length > 0 && data[0].id) {
                    ...
                    // 5. Save the unique profile ID locally (AsyncStorage)
                    await AsyncStorage.setItem("currentProfileId", profileId.toString());
                    console.log("Profile created via REST API and ID saved:", profileId);

                    // 6. Navigate to the next screen (Step 2: Contact Info)
                    navigation.navigate("ContactInfo");
                } else {
                    Alert.alert(
                        "API Error",
                        "Profile was saved but no ID was returned. Check 'Prefer: return=representation' header and RLS SELECT policy."
                    );
                }
            } else {
                ...
                Alert.alert(
                    "Save Error",
                    `Could not save basic info (Status: ${response.status}). Check RLS policy or payload.`
                );
            }
        } catch (e) {
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.card}>
                    ...
                    {/* 2. Gender Dropdown - UPDATED SCROLL PROPS */}
                    <Text style={styles.inputLabel}>
                    ...
                    <DropDownPicker
                        ...
                        scrollViewProps={{
                            ...
                            thumbColor: "#FF7E1D", // ✅ Set orange scroll thumb for Android
                        }}
                        disabled={isLoading}
                    />
                    {genderError ? (
                    ...
                    {/* 3. Date of Birth (Calendar Picker) */}
                    <Text style={styles.inputLabel}>
                    ...
                    {/* 4. Marital Status Dropdown - UPDATED SCROLL PROPS */}
                    <Text style={styles.inputLabel}>
